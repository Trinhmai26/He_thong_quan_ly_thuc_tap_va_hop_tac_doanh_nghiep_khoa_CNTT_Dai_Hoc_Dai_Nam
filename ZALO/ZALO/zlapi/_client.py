...
class ZaloAPI(object):
    ...
    def getgrlink(self, grid):
        ...
        if data.get("error_code") == 0:
            link_info = self._decode(data.get("data"))
            if link_info:
                return {"success": True, "link_info": link_info}
            else:
                return {"success": False, "error_code": 1337, "error_message": "Không tìm thấy thông tin liên kết nhóm."}
        else:
    ...
    def disperse_group(self, grid):
        ...
        response = self._post("https://tt-group-wpa.chat.zalo.me/api/group/disperse", data=params)
        data = response.json()
    ...
    def sendCardBank(self,author_id, message_object, banknum, nameAccBank,bank,bin_code, thread_id, thread_type):
        """Gửi một card bank đến người dùng hoặc nhóm.
        Args:
        banknum (str): số tài khoản
        nameAccBank (str)): Tên chủ tài khoản
        bank (str): loại ngân hàng
    ...
    def remove_friend(self, fid):
        ...
        response = self._post("https://tt-friend-wpa.chat.zalo.me/api/friend/remove", data=params)
        data = response.json()
    ...
    def joinGroup(self, url):
        ...
        response = self._post("https://tt-group-wpa.chat.zalo.me/api/group/link/join", 
                            params=params, 
                            data=payload)
    ...
    extension="vrxx",
    ttl=0,
    fileChecksum=None,   # <-- thêm tham số, mặc định None
):
        """
        Send File to a User/Group with url.
        Tự tính kích thước & MD5 nếu thiếu.
        """

        # Nếu thiếu fileSize hoặc checksum -> lấy từ URL
        if fileSize is None or fileChecksum is None:
            try:
                with self._state._session.get(fileUrl) as response:
                    if response.status_code == 200:
                        content = response.content
                        if fileSize is None:
                            fileSize = int(response.headers.get("Content-Length", len(content)))
                        if fileChecksum is None:
                            fileChecksum = hashlib.md5(content).hexdigest()
                    else:
                ...
                # Không chặn luồng nếu không lấy được; vẫn gửi với giá trị an toàn
                if fileSize is None:
                    fileSize = 0
                if fileChecksum is None:
                    fileChecksum = ""

        # Lấy extension từ fileName nếu có
        has_extension = fileName.rsplit(".")
        extension = has_extension[-1] if len(has_extension) >= 2 else extension
        ...
        payload = {
            "params": {
                ...
                "checksum": fileChecksum,   # <-- dùng đúng key "checksum"
                "checksumSha": "",
                "extension": extension,
        ...
        if results:
            ...
            if isinstance(results, str):
                try:
                    ...
                    results = {"error_code": 1337, "error_message": results}
            return (Group.fromDict(results, None) if thread_type == ThreadType.GROUP
